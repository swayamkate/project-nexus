...
export const Skeleton: React.FC<SkeletonProps> = ({
  ...
  const skeletons = Array.from({ length: count }, (_, i) => (
    <div
      key={i}
      className={`animate-pulse bg-slate-800/80 ${getBaseClasses()} ${className}`}
    />
  ));
...
export const DashboardCardSkeleton: React.FC = () => {
  return (
    <div className="bg-[#0d1527] border border-slate-800 p-6 rounded-2xl shadow-sm space-y-4 animate-pulse">
      ...
      <div className="space-y-2">
        ...
        <div className="w-36 h-8 rounded-lg bg-slate-800" />
      </div>
    </div>
  );
};
