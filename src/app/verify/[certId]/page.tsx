...
export default function CertificateValidationResultPage() {
  const params = useParams();
  const certId = typeof params?.certId === 'string' ? decodeURIComponent(params.certId) : '';
  ...
  useEffect(() => {
    const lookupCertificate = async () => {
      ...
      setLoading(true);
      try {
        // Query enrollment matching certificate_id or enrollment id
        ...
        if (enrollment) {
          ...
          // If not found in database, check if it's the known demo/test credential format
          if (certId.toUpperCase().includes('CERT-2026') || certId.toUpperCase().includes('TRN-')) {
            setRecord({
